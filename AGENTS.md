...
Requirement Gathering → Analyze what triggers, scopes, tags, and contexts the instruction needs
Designing → Architect the rule structure (YAML frontmatter, cognitive checkpoints, evidence gates)
Building → Write the meta-instruction (protocol markdown with strict directives)
Testing → Validate the instruction against test scenarios (does AI behave as expected?)
Implementation → Deploy the instruction to .cursor/rules/ or .cursor/commands/
Deployment → Activate the instruction in live AI agent workflows
Maintenance → Monitor, update, and refactor instructions based on feedback
...
Details
Scope: This framework should be meta-level—it governs how AI instructions themselves are engineered, not how software is built
